...
export default function SettingsPage() {
  ...
  return (
    <section className="space-y-4">
      ...
      {loading ? <LoadingState /> : !business ? <EmptyState label="No business found for this account." /> : !isOwner ? <EmptyState label="Only the business owner can edit settings." /> : (
          ...
          <Card>
            <form onSubmit={onSubmit} className="grid gap-4 lg:grid-cols-2">
              ...
              <div>
                ...
                <Input value={form.working_hours} onChange={(event) => setForm({ ...form, working_hours: event.target.value })} />
              </div>
              <div>
                <Label>AI tone</Label>
                <Input value={form.ai_tone} onChange={(event) => setForm({ ...form, ai_tone: event.target.value })} placeholder="friendly, formal, short" />
              </div>
              ...
              <div className="lg:col-span-2">
                ...
                <Textarea value={form.delivery_policy} onChange={(event) => setForm({ ...form, delivery_policy: event.target.value })} />
              </div>
              <div className="lg:col-span-2">
                <Label>Return policy</Label>
                <Textarea value={form.return_policy} onChange={(event) => setForm({ ...form, return_policy: event.target.value })} />
              </div>
              <div className="lg:col-span-2">
          ...
          <Card>
            ...
            <div className="mt-4 space-y-3 text-sm">
              ...
              <div>
                ...
                <Input type="file" accept=".docx" disabled={uploading} onChange={(event) => onFileChange(event.target.files?.[0])} />
              </div>
              <div>

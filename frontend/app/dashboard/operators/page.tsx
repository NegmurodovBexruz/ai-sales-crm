...
export default function OperatorsPage() {
  ...
  async function copyCode(value?: string | null) {
    ...
    setMessage("Operator code copied.");
  }
  ...
  return (
    <section className="space-y-4">
      <div>
        ...
        <p className="text-sm text-slate-500">Manage Telegram operators for AI handoff and order notifications.</p>
      </div>
      ...
      {!loading && business && (
        <Card>
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="text-sm font-medium text-slate-700">Operator dashboard code</div>
              <div className="mt-1 font-mono text-lg font-semibold text-slate-950">{business.operator_code || "-"}</div>
              <p className="mt-1 text-sm text-slate-500">
                Operator dashboardga kirishi uchun operatorga Operator code va uning Telegram chat ID kerak bo'ladi.
              </p>
            </div>
      ...
      <Card>
        <form onSubmit={onSubmit} className="grid gap-4 lg:grid-cols-4">
          ...
          <div className="flex gap-2 lg:col-span-4">
            <Button disabled={saving}>{saving ? "Saving..." : editingId ? "Save operator" : "Add operator"}</Button>
            {editingId && <SecondaryButton type="button" onClick={resetForm}>Cancel edit</SecondaryButton>}
          </div>
        </form>
      </Card>
            ...
            <tbody>
              {operators.map((operator) => (
                <tr key={operator.id} className="border-b last:border-0">
                  ...
                  <td className="p-3">
                    <Badge tone={operator.is_active ? "green" : "red"}>{operator.is_active ? "Active" : "Inactive"}</Badge>
                  </td>
                  <td className="p-3 text-slate-500">{formatDate(operator.created_at)}</td>
                  <td className="p-3">
                    <div className="flex flex-wrap gap-2">
                      <SecondaryButton onClick={() => startEdit(operator)}>Edit</SecondaryButton>
                      ...
                      <DangerButton onClick={() => run(() => api.deleteOperator(operator.id), "Operator o'chirildi yoki noaktiv qilindi.")}>Delete</DangerButton>
                    </div>
                  </td>
                </tr>
              ))}
